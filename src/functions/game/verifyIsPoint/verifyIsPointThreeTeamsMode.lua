  ...
  for j = 1, quantityBalls do
    ...
    if gameStats.typeMap == "large4v4" and ballStats ~= nil and ballOnGameTwoBalls[j] and ballsId[j] ~= nil and ballOnGame then
      if #teamPointsArea1 > 0 or #teamPointsArea2 > 0 or #teamPointsArea3 > 0 then
        pointsThreeTeamsMode(ballStats.x, ballStats.y, j, "large4v4", nil)
      else
        if ballStats.x <= gameStats.redX and isBallOnGround(ballsId[j]) and teamsLifes[2].red >= 1 and not onCondition and not lostLife then
          lostLife = true
          onCondition = true
    ...
    elseif gameStats.typeMap == "large3v3" and ballOnGameTwoBalls[j] and ballsId[j] ~= nil and ballOnGame then
      for i = 1, #getTeamsLifes do
        if #teamPointsArea1 > 0 or #teamPointsArea2 > 0 then
          pointsThreeTeamsMode(ballStats.x, ballStats.y, j, "large3v3", i)
        else
          if ballStats.x <= 599 and isBallOnGround(ballsId[j]) and getTeamsLifes[1] >= 1 and i == 1 and not lostLife then
            lostLife = true
            ...
            showTheScore()
          elseif ballStats.x >= 601 and isBallOnGround(ballsId[j]) and getTeamsLifes[2] >= 1 and i == 2 and not lostLife then
            lostLife = true
