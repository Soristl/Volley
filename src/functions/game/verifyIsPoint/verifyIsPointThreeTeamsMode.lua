    ...
    elseif gameStats.typeMap == "large3v3" and ballOnGameTwoBalls[j] and ballsId[j] ~= nil then
      for i = 1, #getTeamsLifes do
        if #teamPointsArea1 > 0 or #teamPointsArea2 > 0 then
          ...
          if tfm.get.room.objectList[ballsId[j]].x <= 599 and isBallOnGround(ballsId[j]) and getTeamsLifes[1] >= 1 and i == 1 and not lostLife then
            lostLife = true
            ...
            showTheScore()
          elseif tfm.get.room.objectList[ballsId[j]].x >= 601 and isBallOnGround(ballsId[j]) and getTeamsLifes[2] >= 1 and i == 2 and not lostLife then
            lostLife = true
